...
test('scaffoldSamplePlugin creates plugins/sample_plugin from template', () => {
	...
	assert.equal(targetDir, join(root, 'plugins', 'sample_plugin'));
	assert.equal(existsSync(join(root, 'plugins', 'sample_plugin', 'index.ts')), true);
	assert.equal(readFileSync(join(root, 'plugins', 'sample_plugin', 'index.ts'), 'utf-8'), 'export const sample = true;\n');
});
...
test('scaffoldSamplePlugin rejects existing target', () => {
	...
	const templateDir = join(packageRoot, 'templates', 'gen', 'sample-plugin');
	const targetDir = join(root, 'plugins', 'sample_plugin');
	mkdirSync(templateDir, { recursive: true });
