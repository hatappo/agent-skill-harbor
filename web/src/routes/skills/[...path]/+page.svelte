...
<div class="mx-auto max-w-4xl px-4 py-8 sm:px-6 lg:px-8">
	...
	<nav class="mb-6 text-sm text-gray-500">
		...
		<span class="text-gray-900">{skillName}</span>
	</nav>

	<!-- Header -->
	<div class="mb-8">
		<div class="flex items-start justify-between gap-4">
			<h1 class="text-3xl font-bold text-gray-900">{skillName}</h1>
			<GovernanceBadge status={skill.usagePolicy as UsagePolicy} />
		</div>
		<p class="mt-3 text-lg text-gray-600">{skillDescription}</p>
		<div class="mt-4 flex flex-wrap items-center gap-3">
			<a
				href={repoUrl}
				target="_blank"
				rel="noopener noreferrer"
				class="inline-flex items-center gap-2 rounded-md bg-gray-900 px-4 py-2 text-sm font-medium text-white shadow-sm transition-colors hover:bg-gray-700"
			>
	...
	<div class="mb-8 grid gap-6 sm:grid-cols-2">
		...
		<div class="rounded-lg border border-gray-200 bg-white p-5">
			...
			<dl class="space-y-1.5 text-sm">
				<div class="flex gap-2">
					...
					<dd class="font-medium text-gray-900">{skill.owner}/{skill.repo}</dd>
				</div>
				<div class="flex gap-2">
					<dt class="text-gray-500">Skill path:</dt>
					<dd class="font-medium text-gray-900">{skill.skillPath}</dd>
				</div>
				<div class="flex gap-2">
					<dt class="text-gray-500">Platform:</dt>
					<dd class="font-medium text-gray-900">{skill.platform}</dd>
				</div>
				<div class="flex gap-2">
					<dt class="text-gray-500">Visibility:</dt>
					<dd class="font-medium text-gray-900">{skill.visibility}</dd>
				</div>
				{#if metadata.author}
					<div class="flex gap-2">
						<dt class="text-gray-500">Author:</dt>
						<dd class="font-medium text-gray-900">{metadata.author}</dd>
					</div>
				{/if}
				{#if metadata.version}
					<div class="flex gap-2">
						<dt class="text-gray-500">Version:</dt>
						<dd class="font-medium text-gray-900">{metadata.version}</dd>
					</div>
				{/if}
			</dl>
		</div>
		...
		<div class="rounded-lg border border-gray-200 bg-white p-5">
			...
			{#if skill.note}
				<p class="mt-3 text-sm text-gray-700">{skill.note}</p>
			{/if}
		</div>
	</div>
	...
	{#if fromHistory.length > 0}
		<div class="mb-8 rounded-lg border border-gray-200 bg-white p-5">
			<h2 class="mb-3 text-sm font-medium text-gray-500">Source History</h2>
	...
	{#if excerpt}
		<div class="mb-8 rounded-lg border border-gray-200 bg-white p-5">
			<h2 class="mb-3 text-sm font-medium text-gray-500">Skill Instructions (excerpt)</h2>
			<pre class="whitespace-pre-wrap font-sans text-sm leading-relaxed text-gray-700">{excerpt}</pre>
		</div>
	{/if}
	...
	{#if skill.files && skill.files.length > 0}
		<div class="rounded-lg border border-gray-200 bg-white p-5">
			<h2 class="mb-3 text-sm font-medium text-gray-500">Files ({skill.files.length})</h2>
			<ul class="space-y-1">
				{#each skill.files as file}
					<li>
						...
						>
							<svg class="h-3.5 w-3.5 shrink-0 text-gray-400" viewBox="0 0 20 20" fill="currentColor">
